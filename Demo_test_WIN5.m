...
load(fullfile(folderModel, ['SW_N' num2str(noiseSigma) '.mat']));
 
 
%%% read images
ext         =  {'*.jpg','*.png','*.bmp'};
%ext         =  {'35049.jpg','108036.jpg'};
...
for i = 1:length(filePaths)
   ...
   disp([filePaths(i).name,'   ',num2str(PSNRCur,'%2.2f'),'   ',num2str(SSIMCur,'%2.4f'),'   ',num2str(timeCur,'%2.2f')]);
    ...
    if IsSaveResult
     ...
     imwrite(im2uint8(output),fullfile(savepath,[nameCur '_recover_WIN5_N' num2str(noiseSigma) '.bmp']));
   end
end
